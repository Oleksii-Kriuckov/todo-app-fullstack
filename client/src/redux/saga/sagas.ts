...
function* sagaCreateTodo(action: ICreateAction): Generator<Effect, void> {
  try {
    ...
    yield put(showAlert("Case successfully created", "success"));
    yield call(delay, 3000);
    yield put(hideAlert());
  } catch (error) {
    yield put(showAlert(`Failed to create case: ${error}`, "warning"));
...
function* sagaGetTodos(): Generator<Effect, void, ITodo[]> {
  try {
    ...
    yield put(showAlert("Cases uploaded successfully", "success"));
    yield call(delay, 3000);
    yield put(hideAlert());
  } catch (error) {
    yield put(showAlert(`Failed to load cases: ${error}`, "warning"));
  }
}
...
function* sagaDeleteTodo(action: IDeleteAction): Generator<Effect, void> {
  try {
    ...
    yield put(showAlert("Case successfully delete", "success"));
    yield call(delay, 3000);
    yield put(hideAlert());
  } catch (error) {
    yield put(showAlert(`Failed to delete case: ${error}`, "warning"));
  }
}
...
function* sagaCompleteTodo(action: ICompleteAction<ITodo>): Generator<Effect, void> {
    try {
        ...
        yield put(showAlert(`Case successfully  ${action.payload.done ? 'completed' : 'resumed'} `, 'success'))
        yield call(delay, 3000)
        yield put(hideAlert())
    } catch (error) {
        yield put(showAlert(`Failed to complete the case: ${error}`, 'warning'))
    }
}
...
function* sagaEditTodo(action: IEditAction): Generator<Effect, void, ITodo> {
    try {
        ...
        yield put(showAlert('Case changed successfully', 'success'))
        yield call(delay, 3000)
        yield put(hideAlert())
    } catch (error) {
        yield put(showAlert(`Failed to change the case: ${error}`, 'warning'))
    }
}
